...
随后FCntUp随每个上行链路递增。NFCntDown随FPort 0上的每个下行链路或FPort字段丢失时递增。AFCntDown随着每个下行链路在不同于0的端口上递增。在接收端，相应的计数器与接收到的值保持同步，只要接收到的值与当前的计数器值相比增加了，并且消息MIC字段与使用相应的网络会话密钥在本地计算的MIC值相匹配。如果多次传输确认或未确认的帧，FCnt不会递增（请参阅NbTrans参数）。网络服务器应该丢弃重发帧的应用负载，并且只将单个实例转发给应用服务器。
