...
在下行链路方向存在两个不同的帧计数器方案；单个计数器方案，其中当设备作为LoRaWAN1.0设备操作时，所有端口共享相同的下行链路帧计数器FCntDown；以及双计数器方案，当设备作为LoRaWAN1.1设备运行时，其中单独的NFCntDown用于端口0上的MAC通信，当FPort字段丢失时另一个AFCntDown用于所有其他端口。
